...
} from '../serviceOwners/serviceOwner.dto';
...
export class ServiceOwnersController {
  ...
  async getServiceOwners() {
    ...
    return serviceOwners.map(
      (serviceOwner) => new ServiceOwnerPresenter(serviceOwner)
    );
  }
  ...
  async createServiceOwner(
    @Body() createServiceOwnerDto: CreateServiceOwnerDto
  ) {
    const serviceOwnerCreated = await this.useCase.createServiceOwner(
      createServiceOwnerDto
    );
