...
const FileUpload = (props: FileUploadProps) => {
  const [fileCount, setFileCount] = useState<
    { index: number; value: string; status: 'Added' | 'Removed' }[]
  >([]);
  const ref = useRef([]);
  ...
  const addFileInput = () => {
    if (fileCount.length == 0) {
      ...
      setFileCount([
        ...fileCount,
        { index: fileCount.length, value: '', status: 'Added' },
      ]);
    }
  };
              ...
              <>
                <input
                  ...
                  {...props.register(`files.${item.index}`, {
                    ...
                    onChange: (event) =>
                      setFileCount([
                        ...fileCount.slice(0, item.index),
                        {
                          index: item.index,
                          value: event.target.files[0].name,
                          status: 'Added',
                        },
                        ...fileCount.slice(item.index + 1),
                      ]),
                  })}
                ...
                {item.value != '' && (
                  <li className="tw-flex tw-items-center tw-pr-2 hover:tw-bg-gray-200 visibility-hoverable-container">
                    ...
                    >
                      <IconTrash
                        className="tw-flex hover:tw-cursor-pointer hoverable-visibility-content"
                        size={14}
                        onClick={() =>
                          setFileCount([
                            ...fileCount.slice(0, item.index),
                            { index: item.index, value: '', status: 'Removed' },
                            ...fileCount.slice(item.index + 1),
                          ])
                        }
                      />
