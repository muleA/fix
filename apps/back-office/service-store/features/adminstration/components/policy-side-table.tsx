  ...
  return (
    <Card className="tw-w-4/12 tw-ml-4" shadow="sm" padding="lg">
      <Card.Section className="tw-flex tw-justify-between tw-border-b tw-py-2 tw-px-4 tw-mb-2">
        <h2 className="tw-text-lg">policy</h2>
        <Link href="/service-store/adminstration/new-policy">
          ...
          <tbody className="tw-border-b">
            ...
            {Service.length > 0 &&
              Service.map((item) => (
                <Link
                  key={item.name}
                  href={`/service-store/adminstration/details/policy/${item.name}`}
                >
...
export default AdminstrationSideTable;
